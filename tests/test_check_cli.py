...
def test_check_prints_top_plots_only_for_first_matching_run(capsys):
    ...
    assert "0001 \x1b[31m✗ FAIL\x1b[0m  [pwg-rmngrid run 1 |" in captured.out
    assert "0002 \x1b[31m✗ FAIL\x1b[0m  [pwg-rmngrid run 2 |" not in captured.out
    assert any(
        line.startswith("0001 \x1b[31m✗ FAIL\x1b[0m  ") and "┌" in line
        for line in lines
    )
